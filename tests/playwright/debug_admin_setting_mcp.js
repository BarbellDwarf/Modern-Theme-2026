...
(async ()=>{
  ...
  const out = path.resolve(__dirname, '../../screenshots/admin_setting.html');
  const log = path.resolve(__dirname, '../../screenshots/admin_setting.log');
  ...
  try{
    ...
    await page.goto(base + '/admin/setting', { waitUntil: 'networkidle' });
    await page.waitForTimeout(1000);
    const html = await page.content();
    fs.writeFileSync(out, html);
    logLine('wrote admin settings page to ' + out);
    await browser.close();
  }catch(e){
