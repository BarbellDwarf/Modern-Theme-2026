...
(async () => {
  ...
  const log = path.join(outdir, 'messenger_send_mcp_v4.log');
  ...
  try {
    ...
    await page.screenshot({ path: path.join(outdir, 'messenger_v4_after_login.png') });

    // Go to mail index
    await page.goto(base + '/mail/mail/index', { waitUntil: 'networkidle' });
    await page.waitForTimeout(800);
    await page.screenshot({ path: path.join(outdir, 'messenger_v4_index.png') });
    ...
    // Wait for composer or navigate to create URL if needed
    ...
    if (!composer) {
      logLine('Composer not found; abort');
      await page.screenshot({ path: path.join(outdir, 'messenger_v4_no_composer.png') });
    } else {
      logLine('Composer visible');
      await page.screenshot({ path: path.join(outdir, 'messenger_v4_composer.png') });
      ...
      // Attempt to set textarea via evaluate (handles hidden textarea or WYSIWYG wrappers)
      const messageText = 'Automated MCP mobile v4 message at ' + new Date().toISOString();
      ...
      // Click send via DOM evaluate (bypasses viewport)
      ...
      await page.screenshot({ path: path.join(outdir, 'messenger_v4_after_send.png') });
    }
